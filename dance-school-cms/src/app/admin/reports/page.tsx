...
async function getReportData(): Promise<ReportData> {
  try {
    ...
    const bookingRevenue = completedBookings.reduce((sum: number, b: any) => sum + (b.amount || 0), 0);
    const subscriptionRevenue = subscriptions.reduce((sum: number, s: any) => sum + (s.purchasePrice || 0), 0);
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthSubscriptions = subscriptions.filter((s: any) => {
        const subscriptionDate = new Date(s.startDate);
        return subscriptionDate >= monthStart && subscriptionDate <= monthEnd;
      });
      
      const monthlyBookingRevenue = monthBookings.reduce((sum: number, b: any) => sum + (b.amount || 0), 0);
      const monthlySubscriptionRevenue = monthSubscriptions.reduce((sum: number, s: any) => sum + (s.purchasePrice || 0), 0);
      
      revenueByMonth.push({
        month: format(monthStart, 'MMM yyyy'),
        revenue: monthlyBookingRevenue + monthlySubscriptionRevenue,
        bookings: monthBookings.length + monthSubscriptions.length
      });
    }
    ...
    completedBookings.forEach((booking: any) => {
      ...
      if (classId) {
        ...
        classBookings[classId].revenue += (booking.amount || 0);
      }
    });
    ...
    const recentActivity = [
      ...
      ...subscriptions.slice(-10).map((s: any) => ({
        type: 'subscription',
        description: `New subscription: ${s.passName || 'Unknown Pass'} by ${s.user?.name || 'Unknown User'}`,
        date: s.startDate,
        amount: s.purchasePrice
      })),
      ...recentUsers.slice(-5).map((u: any) => ({
        type: 'user',
