...
const client = sanityClient({
  projectId: 'a2qsy4v6', // replace with your project ID
  dataset: 'production',        // or your dataset name
  token: 'sk0oThJpx2nkFl2e1CzgNPXk0501sf2mfP9hg8mLn1hKBwdj8rOdlR7KB3uyB1kg3sZCBgPMw8XcCQzpKiwhpGmL5QdOpU7s9Y1uPpU0TT7qUkhRZvNvxfZTqtffCzrxEoBBQ8J68ImwtAhTrV9oO7orwv1qM7184iOMqEKAJx9CqwWjN2Ry', // replace with your token
...
async function createAdminRoleAndAssignUser() {
  try {
    ...
    const existingAdmin = await client.fetch('*[_type == "role" && name == "Admin"][0]')
    let adminRoleId
    ...
    const tenant = await client.fetch('*[_type == "tenant" && slug.current == $slug][0]', { slug: tenantSlug })
    ...
    const existingUserRole = await client.fetch(`
      *[_type == "userRole" && user._ref == $userId && tenant._ref == $tenantId && role._ref == $roleId][0]
    `, {
      userId: user._id,
      tenantId: tenant._id,
      roleId: adminRoleId
